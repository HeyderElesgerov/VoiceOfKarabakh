...
(function ($) {
    ...
    $.fn.spartanMultiImagePicker = function (options, photoFiles) {
        ...
        if (photoFiles) {
            ...
            for (let photoFile of photoFiles) {
                var url = window.origin + '/' + photoFile.filePath;
                ...
                fetch(url)
                    .then(response => {
                        ...
                        response.blob()
                            .then(result => {
                                const file = new File([result], photoFile.fileName, { type: contentType })
                                dataTransfer.items.add(file);
                            })
                            .then(() => {
                                let newInput = document.getElementsByName(settings.fieldName)[count-1];
                                newInput.files = dataTransfer.files;
                                loadImage(settings, newInput, that);
                                ...
                                $(this).on("click", ".spartan_remove_row", function () {
                                    removeRow(settings, newInput, that);
                                });
                                ...
                                $(this).on("drop", '.spartan_item_wrapper', function (event) {
                                    event.stopPropagation();
                                    event.preventDefault();
                                    onDropImage(settings, newInput, that, event);
                                });
                            });
                    })
            }
